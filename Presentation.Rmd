...
$$T(t)\phi = \sum_{j\in\mathbb{N}}e^{-\lambda^{\alpha/2}_jt}\left(\phi, e_j\right)_{L_2(\Gamma)}e_j.\tag{SG}$$
...
$$u(t) = T(t)u_0 + \int_0^t T(t-r)f(r)dr,\quad t\geq 0.\tag{MS}$$
]
...
Using the spectral representation (SG) of the semigroup $T(t)$, the solution (MS) of problem $(3)$ can be expressed as
...
$$\|u-U_{h,m}^\tau\|_{L_2((0,T);L_2(\Gamma))}\lesssim\tau+h^\alpha + h^{\alpha-2}e^{-2\pi\sqrt{(1-\alpha/2)m}}$$
...
and solve for the other two parameters when one is given.
...
$$\|u-U_{h,m}^\tau\|_{L_2((0,T);L_2(\Gamma))}\lesssim C_h h^\alpha$$
...
$$\min\; J(u, z),\quad J(u, z)=  \frac{1}{2}\int_0^T\left(\left\|u-u_d\right\|_{L_2(\Gamma)}^2+\mu\|z\|_{L_2(\Gamma)}^2\right) dt\tag{18}$$
...
.content-box-gray[
A triple $(u, p, z)$ is optimal for $(18)$-$(20)$ if and only if $u$ and $p$ satisfy the state and adjoint equations, respectively, and $z$ satisfies \eqref{proj_formula}.
]
...
.content-box-gray[
Let $\alpha\in[0,2]$ and $\bar{u}$ be the solution to $(19)$ corresponding to the optimal control $\bar{z}$ of $(18)$ - $(20)$. Let $\bar{U}_{h,m}^\tau$ denote the piecewise linear approximation of the optimal discrete state obtained from the discrete scheme after the rational approximation. Let $\bar{Z}_{h,m}^\tau$ denote the corresponding piecewise constant approximation of the optimal discrete control. If $f,u_d\in L_2((0,T);\dot{H}^\beta(\Gamma)) \cap L_\infty((0,T);L_2(\Gamma))$ for $\beta<1$ and $u_0\in\dot{H}^{\alpha}(\Gamma)$ , then

$$\|\bar{u}-\bar{U}_{h,m}^\tau\|_{L_2((0,T);L_2(\Gamma))}\lesssim\tau+h^\alpha + h^{\alpha-2}e^{-2\pi\sqrt{(1-\alpha/2)m}}$$

and

$$\|\bar{z}-\bar{Z}_{h,m}^\tau\|_{L_2((0,T);L_2(\Gamma))}\lesssim\tau+h^\alpha + h^{\alpha-2}e^{-2\pi\sqrt{(1-\alpha/2)m}}$$
]
...
3 *Adjoint solve:* Given $\mathbf{\bar{U}}$ from \ref{statesolve}, compute $\mathbf{\bar{P}}\in \mathbb{R}^{N_{h}\times (N+1)}$ with the iteration
$$
\begin{cases}
...
4 *Control update:* Given $\mathbf{\bar{P}}$ from \ref{adjointsolve}, update 
...
The unfolded iterations in \eqref{numericalscheme1} and \eqref{thenumericalscheme2} can equivalently be written in block-matrix form as


```{r, echo=FALSE, fig.align='center', out.width='70%'}
# figures
knitr::include_graphics("data_files/matrix_form.png")
```
...
\end{align*}
\begin{align*}
    \mathbf{\bar{z}}^{(n+1)}  = \mathfrak{T}(\mathbf{\bar{z}}^{(n)})
\end{align*}
...
\end{align*}
\begin{align}
\label{explicit_iteration}
    \mathbf{\bar{Z}}^{(n+1)}  = \mathcal{T}(\mathbf{\bar{Z}}^{(n)}).
\end{align}
...
\end{align}
From the Lipschitz property of the componentwise projection operator, we have
\begin{align}
...
Define a scalar matrix $\mathbf{T}$ as $\mathbf{T}  = \begin{bmatrix}\omega^2+\omega^4+\omega^6 & \omega^3+\omega^5 & \omega^4\\\omega^3+\omega^5 & \omega^2+\omega^4 & \omega^3\\\omega^4 & \omega^3 & \omega^2\\\end{bmatrix},\quad \omega = \dfrac{1}{1+\tau\kappa^{2\beta}}<1.$
