...
tau_vector <- 10^seq(log10(0.005), log10(0.1), length.out = 7)
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(tau_vector)) {
    ...
    h <- time_step^(1/alpha)
    m <- min(8, ceiling((log(h))^2 / (pi^2 * (1 - alpha / 2)))) 
...
save(errors_projected, file = here::here("data_files/new_errors_projected_tau.RData"))
```

```{r}
# Load the errors_projected data
load(here::here("data_files/new_errors_projected_tau.RData"))
```
...
```{r}
ggsave(here::here("data_files/new_conv_rates_tau.png"), width = 4, height = 5, plot = p_tau, dpi = 300)
...
# option 1
slackr_upload(
  filename = "data_files/new_conv_rates_tau.png",        # path to your image
