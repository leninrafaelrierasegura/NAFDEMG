...
## Eigenfunction-based construction of an exact solution
...
[e_1 e_2 \dots e_{N_*}] \iff&\quad  \texttt{EIGENFUN}\\
u_0(s) \iff&\quad  \texttt{U_0 <- EIGENFUN %*% coeff_U_0}\\
f(s,t) \iff&\quad  \texttt{FF_true <- EIGENFUN %*% outer(1:length(coeff_FF), 1:length(time_seq), function(i, j) coeff_FF[i] * g_sin(r = time_seq[j], A = AAA, omega = OMEGA))}\\
u(s,t) \iff&\quad  \texttt{U_true <- EIGENFUN %*% outer(1:length(coeff_U_0), 1:length(time_seq), function(i, j) (coeff_U_0[i] + coeff_FF[i] * G_sin(t = time_seq[j], A = AAA, lambda_j_alpha_half = EIGENVAL_ALPHA[i], omega = OMEGA)) * exp(-EIGENVAL_ALPHA[i] * time_seq[j]))}
\end{aligned}
...
\end{equation}
Let $\boldsymbol{F}$ be the matrix with columns $\boldsymbol{F}^k$. Then $\boldsymbol{F}$ and $\boldsymbol{U}^{k+1}$ can be implemented in $\texttt{R}$ as
\begin{aligned}
\text{expression} \iff&\quad \text{In } \texttt{R}\\
\boldsymbol{F}\iff&\quad  \texttt{FF_innerprod <- t(graph\$fem_basis(overkill_graph\$get_mesh_locations())) %*% overkill_graph\$mesh\$C %*% overkill_EIGENFUN %*% outer(1:length(coeff_FF), 1:length(time_seq), function(i, j) coeff_FF[i] * g_sin(r = time_seq[j], A = AAA, omega = OMEGA))}\\
