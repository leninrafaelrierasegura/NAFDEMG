...
# 
#   return(list(xaxis = list(title = list(text = "x", font = list(color = "gray", size = 16)),  tickfont = list(color = "gray", size = 13), range = x_range),
#               yaxis = list(title = list(text = "y", font = list(color = "gray", size = 16)),  tickfont = list(color = "gray", size = 13), range = y_range),
#               zaxis = list(title = list(text = "z", font = list(color = "gray", size = 16)),  tickfont = list(color = "gray", size = 13), range = z_range),
...
#                            substring(sprintf("%.20f", theoretical_rates), 1, ifelse(apply_sqrt, 7, 6)),
#                            #formatC(theoretical_rates, format = "f", digits = 4),
#                            formatC(observed_rates, format = "f", digits = 4),
...
#     theme(
#   text = element_text(family = "Palatino", size = 18),
# 
...
# 
#   # ---- Scene ----
#   p <- p %>% layout(font = list(family = "Palatino Linotype"),
#     scene = global.scene.setter.rec.and.sphere(x_range, y_range, z_range)
#   )
# 
#   return(p)
...
## -----------------------------------------------------------------------------------------------------------------------
# combine_pngs_with_gap <- function(png_files,
#                                   output_file,
#                                   gap = 20,
#                                   align = c("center", "top", "bottom")) {
# 
...
# 
# 
# solve_htau <- function(alpha, m, epsilon, tol = 1e-16) {
# 
...
#   err1 <- abs(tau - h^(alpha - epsilon) * abs(log(tau)))
...
# }
# 
# solve_htaunew <- function(alpha, m, epsilon, tol = 1e-16) {
# 
#   stopifnot(alpha > 0, alpha < 2)
#   stopifnot(m > 0)
# 
...
# }
# 
# tau_from_h <- function(h, alpha, C) {
#   Cmin <- 1  - alpha * log(h)
#   if (any(C < Cmin)) stop("No real solution exists because C < 1 + log(2) - alpha*log(h).")
#   x <- -1 * exp(-C) / h^alpha
#   return(-(h^alpha / 1) * lamW::lambertW0(x))
# }
# 
# # tau_from_h <- function(h, alpha, C) {
# #   Cmin <- 1 + log(2) - alpha * log(h)
# #   if (any(C < Cmin)) stop("No real solution exists because C < 1 + log(2) - alpha*log(h).")
# #   x <- -2 * exp(-C) / h^alpha
# #   return(-(h^alpha / 2) * lamW::lambertW0(x))
# # }
# 
# m_from_h <- function(h, alpha, C, tau) {
#   arg <- h^2 * ((C - abs(log(tau))) / 2)
#   if (arg <= 0) stop("The logarithm argument must be positive.")
#   return(ceiling(log(arg)^2 / (4 * pi^2 * (1 - alpha / 2))))
# }
# 
# # # Parameters
# # h <- 0.05
...
# #
# # # Compute tau
# # tau <- tau_from_h_exact(h, alpha, C)
# #
# # # Left- and right-hand sides
# # lhs <- tau + 0.5 * h^alpha * abs(log(tau))
# # rhs <- 0.5 * C * h^alpha
# #
# # # Print results
# # cat("tau =", tau, "\n")
# # cat("LHS =", lhs, "\n")
# # cat("RHS =", rhs, "\n")
# # cat("Absolute error =", abs(lhs - rhs), "\n")
# # cat("Relative error =", abs(lhs - rhs) / abs(rhs), "\n")
