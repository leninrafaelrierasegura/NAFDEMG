...
for (j in 1:length(alpha_vector)) {
  ...
  xgh <- range(overkill_elliptic_v) 
  ygh <- range(overkill_phi)

  vals <- c(xgh[1] * ygh[1], xgh[1] * ygh[2],
          xgh[2] * ygh[1], xgh[2] * ygh[2])
  ...
  for (i in 1:length(tau_vector)) {
    ...
    m <- min(m_cut, ceiling((log(h^(2 - epsilon) * time_step^2 * abs(log(time_step))))^2 / (4 * pi^2 * (1 - alpha / 2))))
    ...
    auxX <- res$U
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_u_bar - auxX
    UU <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX))))
    
    auxX <- res$P
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_p_bar - auxX
    PP <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX))))
    
    auxX <- res$Z
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_z_bar - auxX
    ZZ <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX))))
    ...
    slackr_msg(text = paste0("m =", m, ", alpha =", alpha, ", h =", h, ", time_step =", time_step, ", converged = ", res$converged, ", iterations = ", res$iterations, ", error u = ", UU, ", error p = ", PP, ", error z = ", ZZ, ", minp = ", minp, ", maxp = ", maxp), channel = "#research")
  }
}
save(errors_u_bar, errors_p_bar, errors_z_bar, file = here::here("data_files/control_error_tau_with_iteration_with_logtau_and_tauminustwo.RData"))
```
# Convergence results

```{r}
# Load the errors data
load(here::here("data_files/control_error_tau_with_iteration_with_logtau_and_tauminustwo.RData"))
```
...
p_u_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
...
p_p_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                               alpha_vector, 
                               errors_p_bar, 
                               theoretical_rates, 
...
p_z_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
...
p_tau_u_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
                               observed_rates_u_bar,
...
p_tau_u_nicer_tadpole_with_logtau_and_tauminustwo <- errorConvergencePlotterLatex(x_axis_vector = tau_vector, 
                                 alpha_vector, 
                                 errors_u_bar, 
                                 theoretical_rates, 
                                 observed_rates_u_bar,
...
p_tau_z_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
                               observed_rates_z_bar,
...
p_tau_z_nicer_tadpole_with_logtau_and_tauminustwo <- errorConvergencePlotterLatex(x_axis_vector = tau_vector, 
                                 alpha_vector, 
                                 errors_z_bar, 
                                 theoretical_rates, 
                                 observed_rates_z_bar,
...
p_all_tau_with_logtau_and_tauminustwo <- (p_u_bar_with_logtau_and_tauminustwo | p_p_bar_with_logtau_and_tauminustwo | p_z_bar_with_logtau_and_tauminustwo) + 
  plot_annotation(
...
myggsave(p_tau_u_nicer_tadpole_with_logtau_and_tauminustwo, width = 4, height = 5, use_mathpazo = TRUE)
save(p_tau_u_nicer_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_u_nicer_tadpole_with_logtau_and_tauminustwo.RData"))

myggsave(p_tau_z_nicer_tadpole_with_logtau_and_tauminustwo, width = 4, height = 5, use_mathpazo = TRUE)
save(p_tau_z_nicer_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_z_nicer_tadpole_with_logtau_and_tauminustwo.RData"))

save(p_tau_u_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_u_with_logtau_and_tauminustwo.RData"))
save(p_tau_z_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_z_with_logtau_and_tauminustwo.RData"))

ggsave(here::here("data_files/control_conv_rates_tau_all_with_iteration_with_logtau_and_tauminustwo.png"), width = 12, height = 6, plot = p_all_tau_with_logtau_and_tauminustwo, dpi = 300)
```
...
knitr::include_graphics(here::here("data_files/control_conv_rates_tau_all_with_iteration_with_logtau_and_tauminustwo.png"))
```
...
# option 1
slackr_upload(
  filename = "data_files/control_conv_rates_tau_all_with_iteration_with_logtau_and_tauminustwo.png",        # path to your image
