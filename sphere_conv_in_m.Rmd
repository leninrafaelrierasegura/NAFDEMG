---
title: "Convergence in 𝑚, sphere case"
...
save(errors_projected, file = here::here("data_files/errors_projected_m_sphere.RData"))
```


```{r}
load(here::here("data_files/errors_projected_m_sphere.RData"))
```
...
p_m_sphere <- error.convergence.plotter(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
p_m_nicer_sphere <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
                                 ...
                                 y_axis_label = "$\\|u-U_{h,m}^{\\tau}\\|_{L_2(\\mathbb{S}^2\\times[0,T])}\\quad(\\log_{\\mathrm{e}}\\mbox{ scale})$",
                                 color_label = "$\\quad\\quad\\;\\;\\alpha\\;|\\;\\phantom{-}\\mathtt{theo}\\;|\\;\\;\\phantom{-}\\mathtt{obs}\\;\\;|\\%\\mathtt{re}$",
                                 apply_sqrt = TRUE)

ggsave(here::here("data_files/conv_rates_m_sphere.png"), width = 4, height = 5, plot = p_m_sphere, dpi = 300)
myggsave(p_m_nicer_sphere, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_sphere, file = here::here("data_files/p_m_sphere.RData"))
save(p_m_nicer_sphere, file = here::here("data_files/p_m_nicer_sphere.RData"))
```


```{r, eval = TRUE, fig.align='center', out.width="400px", out.height="500px", fig.cap = captioner("Comparison between theoretical and observed convergence behavior for the $L_2(\\mathbb{S}^2\\times(0,T))$-error with respect to $m$ on a semi-$\\text{log}_{e}$ scale, with $m$ plotted on a square-root scale. Dashed lines indicate the theoretical rates, and solid lines represent the observed error curves. The legend below each plot shows the value of $\\alpha$ along with the corresponding theoretical ('theo'), and observed ('obs') rates for each case.")}
knitr::include_graphics(here::here("data_files/tikzpic/p_m_nicer_sphere_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = "data_files/conv_rates_m_sphere.png",        # path to your image
