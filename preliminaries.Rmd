...
A function $U_{\text{coarse}}(s)$ defined on a coarse mesh with $N_{h}$ nodes can be projected onto a fine mesh with $N_{h_{\text{ok}}}$ nodes by doing $U_{\text{fine}}(s) = \boldsymbol{A} U_{\text{coarse}}(s)$, where $\boldsymbol{A}$ is a matrix with entries $\psi^j_h(s_i)$ for $j=1\dots, N_h$ and $i = 1,\dots N_{h_{\text{ok}}}$.
