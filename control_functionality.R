...
solve_coupled_system_multi_tol <- function(
  ...
  z_prev <- Z_ini
  Z_mat <- ifelse(nested_spatial_mesh, C %*% z_prev, R %*% Psi %*% z_prev)
  U_prev <- F_proj*0
  P_prev <- F_proj*0
  ...
  repeat {
    ...
    V_mat <- ifelse(nested_spatial_mesh, C %*% U_mat, R %*% Psi %*% U_mat)
    Q_mat <- solve_backward_evolution(my_op_frac, time_step, time_seq, RHST = V_mat - V_d)
    P_mat <- Q_mat
    z_new <- pmax(A, pmin(B, - P_mat / mu))
    Z_mat <- ifelse(nested_spatial_mesh, C %*% z_new, R %*% Psi %*% z_new)
