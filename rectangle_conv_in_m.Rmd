...
p_m_square <- error.convergence.plotter(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
p_m_nicer_square <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
                                 ...
                                 y_axis_label = "$\\|u-U_{h,m}^{\\tau}\\|_{L_2([0.1]^2\\times[0,T])}\\quad(\\log_{\\mathrm{e}}\\mbox{ scale})$",
                                 color_label = "$\\quad\\quad\\;\\;\\alpha\\;|\\;\\phantom{-}\\mathtt{theo}\\;|\\;\\;\\phantom{-}\\mathtt{obs}\\;\\;|\\%\\mathtt{re}$",
                                 apply_sqrt = TRUE)

ggsave(here::here("data_files/conv_rates_m_square.png"), width = 4, height = 5, plot = p_m_square, dpi = 300)
myggsave(p_m_nicer_square, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_square, file = here::here("data_files/p_m_square.RData"))
save(p_m_nicer_square, file = here::here("data_files/p_m_nicer_square.RData"))
```


```{r, eval = TRUE, fig.align='center', out.width="400px", out.height="500px", fig.cap = captioner("Comparison between theoretical and observed convergence behavior for the $L_2([0.1]^2\\times(0,T))$-error with respect to $m$ on a semi-$\\text{log}_{e}$ scale, with $m$ plotted on a square-root scale. Dashed lines indicate the theoretical rates, and solid lines represent the observed error curves. The legend below each plot shows the value of $\\alpha$ along with the corresponding theoretical ('theo'), and observed ('obs') rates for each case.")}
knitr::include_graphics(here::here("data_files/tikzpic/p_m_nicer_square_with_mathpazo.pdf"))
```
