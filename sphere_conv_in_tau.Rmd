...
p_tau_sphere <- error.convergence.plotter(x_axis_vector = tau_vector, 
                                   alpha_vector, 
                                   errors_projected, 
                                   theoretical_rates, 
                                   observed_rates,
...
p_tau_nicer_sphere <- errorConvergencePlotterLatex(x_axis_vector = tau_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
                                 ...
                                 y_axis_label = "$\\|u-U_{h,m}^{\\tau}\\|_{L_2(\\mathbb{S}^2\\times[0,T])}\\quad(\\log_{10}\\mbox{ scale})$",
                                 color_label = "$\\quad\\quad\\;\\;\\alpha\\;|\\;\\mathtt{theo}\\;|\\;\\;\\mathtt{obs}\\;\\;|\\%\\mathtt{re}$")

ggsave(here::here("data_files/conv_rates_tau_sphere.png"), width = 4, height = 5, plot = p_tau_sphere, dpi = 300)
myggsave(p_tau_nicer_sphere, width = 4, height = 5, use_mathpazo = TRUE)
save(p_tau_sphere, file = here::here("data_files/p_tau_sphere.RData"))
save(p_tau_nicer_sphere, file = here::here("data_files/p_tau_nicer_sphere.RData"))
```


```{r, eval = TRUE, fig.align='center', out.width="400px", out.height="500px", fig.cap = captioner("Comparison between theoretical and observed convergence behavior for the $L_2(\\mathbb{S}^2\\times(0,T))$-error with respect to $\\tau$ on a $\\text{log}_{10}$–$\\text{log}_{10}$ scale. Dashed lines indicate the theoretical rates, and solid lines represent the observed error curves. The legend below each plot shows the value of $\\alpha$ along with the corresponding theoretical ('theo'), and observed ('obs') rates for each case.")}
knitr::include_graphics(here::here("data_files/tikzpic/p_tau_nicer_sphere_with_mathpazo.pdf"))
```
