...
E <- overkill_time_step * kronecker(Diagonal(n = length(overkill_time_seq)), overkill_graph$mesh$C)
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    xx <- as.vector(overkill_U_true - projected_U_piecewise)
    errors_projected[i,j] <- sqrt(as.double(t(xx) %*% E %*% xx)) 
