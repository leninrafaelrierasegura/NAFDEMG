...
min_hs <- min(hs)
print(paste0("min of all possible hs = ", min_hs))

min_taus <- min(taus)
print(paste0("min of all possible taus = ", min_taus))
# Truncate to first significant digit
h_star <- trunc_first_signi_digit(min_hs)
print(paste0("h_star = ", h_star))

tau_star <- h_star^2
print(paste0("tau_star = ", tau_star))
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    h <- exp(- pi * sqrt((1 - alpha / 2) * m))/divider 
    time_step <- (h^alpha)
    h_aux <- largest_nested_h(h_star, h)
    tau_aux <- largest_nested_h(tau_star, time_step)
    print(paste0("m = ", m, ", h_cand = ", formatC(h, format = "f", digits = 6), ", h_aux = ", h_aux, ", h_aux/h_fine = ", h_aux/h_star, ", tau_cand = ", formatC(time_step, format = "f", digits = 6), ", tau_aux = ", tau_aux))
  }}
...
ggsave(here::here("data_files/conv_rates_m_tadpole.png"), width = 4, height = 5, plot = p_m_tadpole, dpi = 300)
myggsave(p_m_nicer_tadpole, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_tadpole, file = here::here("data_files/p_m_tadpole.RData"))
save(p_m_nicer_tadpole, file = here::here("data_files/p_m_nicer_tadpole.RData"))
```
...
knitr::include_graphics(here::here("data_files/tikzpic/p_m_nicer_tadpole_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = "data_files/conv_rates_m_tadpole.png",        # path to your image
