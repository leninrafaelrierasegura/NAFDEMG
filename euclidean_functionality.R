...
## --------------------------------------------------------------------------------------------------------------------------------------------------
flattener <- function(M) as.vector(t(M[nrow(M):1,]))
unflattener <- function(v, nrow, ncol) {
  # Step 1: reshape into transposed size (ncol × nrow)
  MrevT <- matrix(v, nrow = ncol, ncol = nrow, byrow = FALSE)
  
  # Step 2: transpose
  Mrev <- t(MrevT)
  
  # Step 3: reverse rows back to original order
  M <- Mrev[nrow:1, ]
...
def fem_shifted_laplacian_sparse(a, b, nx, ny, kappa=1.0):
    nx = int(nx)
    ny = int(ny)
    ...
    M = assemble(m_form)
    import scipy.sparse
    A_sp = scipy.sparse.csr_matrix(A.array())
    M_sp = scipy.sparse.csr_matrix(M.array())
    return {'L': A_sp, 'C': M_sp}
")
...
compute_true_eigen_rectangle <- function(a = 1,
  ...
  # Mesh grid
  x <- seq(0, a, length.out = nx+1)
  y <- seq(0, b, length.out = ny+1)
