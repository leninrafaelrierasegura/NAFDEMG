---
title: "Paper 1 simulation study"
...
#rspde_fit_stat <- list_models_before_cv$rspde_fit_statnu0.5
...
#rspde_fit_stat <- list_models_before_cv$rspde_fit_statnu1.5
...
#rspde_fit_stat <- list_models_before_cv$rspde_fit_statnuest
...
list_models_before_cv <- list(mean_and_mode_params_statnu0.5 = mean_and_mode_params_statnu0.5,
              ...
              distance = distance,
              GROUPS = GROUPS)
save(list_models_before_cv, file = here::here("old/cv_models.RData"))
```
...
small_neighborhood <- GROUPS[[8]][[point_of_interest]]
large_neighborhood <- GROUPS[[5]][[point_of_interest]]
p <- graph$plot(vertex_size = 0) +
  #geom_point(data = data, aes(x = .coord_x, y = .coord_y), color = "black", size = 2) +
...
```{r, eval = TRUE}
load(here::here("old/cv_models.RData"))
...
save(list_to_save, file = here::here("old/cv_result.RData"))
```


```{r}
load(here::here("old/cv_result.RData"))
################### Plot the results ##########################
...
# Save combined plot
ggsave(here::here("old/cv_sim.png"), plot = combined_plot, width = 9.22, height = 4.01, dpi = 500)
...
```{r, eval = TRUE}
nu_estimated <- round(mean_and_mode_params_statnuest[4,1], 3)
...
slackr_upload(
  filename = here::here("old/cv_sim.png"),        # path to your image
  initial_comment = "here is the plot from the cv simulation study",
  channels = "#research"
)
```
