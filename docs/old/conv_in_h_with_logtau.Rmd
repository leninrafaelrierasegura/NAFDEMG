...
h_vector <- time_steps^(1/2) #10^seq(log10(0.005), log10(0.1), length.out = 7) #time_steps^(1/2)
alpha_vector <- seq(1, 1.8, by = 0.2)
epsilon <- 1e-10
# C_c <- max(outer(h_vector, alpha_vector,
#                function(h, alpha) 1 - alpha * log(h))) + 1e-1
# 
# C_vector <- sapply(alpha_vector, function(alpha) {
#   max(1 - alpha * log(h_vector)) + 1e-1
# })
# 
# hs <- c()
# taus <- c()
...
#     h <- h_vector[i]
#     time_step <- tau_from_h(h, alpha, C_c)
#     m <- min(8, m_from_h(h, alpha, C_c, time_step))
#     hs <- c(hs, h)
#     taus <- c(taus, time_step)
#     ms <- c(ms, m)
#   }}
```
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(h_vector)) {
    ...
    m <- min(4, ceiling((log(h^(2 - epsilon) * abs(log(time_step))))^2 / (4 * pi^2 * (1 - alpha / 2)))) #min(8, m_from_h(h, alpha, C_c, time_step)) 
    ...
    errors_projected[i,j] <- sqrt(as.double(overkill_time_step * sum(XX * (overkill_C %*% XX)))) / abs(log(time_step))
    slackr_msg(text = paste0("m =", m, ", alpha =", alpha, ", h =", h, ", time_step =", time_step), channel = "#research")
  }
}
save(errors_projected, file = here::here("data_files/errors_projected_h_with_logtau.RData"))
```


```{r}
# Load the errors_projected data
load(here::here("data_files/errors_projected_h_with_logtau.RData"))
```
...
p_h_tadpole_with_logtau <- error.convergence.plotter(x_axis_vector = h_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
p_h_nicer_tadpole_with_logtau <- errorConvergencePlotterLatex(x_axis_vector = h_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
ggsave(here::here("data_files/conv_rates_h_tadpole_with_logtau.png"), width = 4, height = 5, plot = p_h_tadpole_with_logtau, dpi = 300)
myggsave(p_h_nicer_tadpole_with_logtau, width = 4, height = 5, use_mathpazo = TRUE)
save(p_h_tadpole_with_logtau, file = here::here("data_files/p_h_tadpole_with_logtau.RData"))
save(p_h_nicer_tadpole_with_logtau, file = here::here("data_files/p_h_nicer_tadpole_with_logtau.RData"))
```
...
knitr::include_graphics(here::here("data_files/tikzpic/p_h_nicer_tadpole_with_logtau_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = here::here("data_files/conv_rates_h_tadpole_with_logtau.png"),        # path to your image
  initial_comment = initial_comment,
  channels = "#research"
)
# option 2
# ggslackr(p_h_tadpole, channels = "#research", initial_comment = initial_comment) # p is a ggplot2 object
# option 3
