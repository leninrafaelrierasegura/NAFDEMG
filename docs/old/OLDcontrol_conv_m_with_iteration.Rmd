...
```{r}
capture.output(
  knitr::purl(here::here("old/OLDfunctionality.Rmd"), output = here::here("old/OLDfunctionality.R")),
  file = here::here("purl_log.txt")
)
source(here::here("old/OLDfunctionality.R"))
```
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    A_aux <- matrix(a, nrow = length(overkill_weights), ncol = length(time_seq))
    B_aux <- matrix(b, nrow = length(overkill_weights), ncol = length(time_seq))
    ...
    slackr_msg(text = paste0("m =", m, ", alpha =", alpha, ", h =", h, ", time_step =", time_step, ", converged = ", res$converged, ", iterations = ", res$iterations, ", error u = ", DDD, ", error p = ", HHH, ", error z = ", KKK, ", error u = ", errors_u_bar[i,j], ", error p = ", errors_p_bar[i,j], ", error z = ", errors_z_bar[i,j]), channel = "#research")
  }
}
...
# option 1
slackr_upload(
  filename = here::here("data_files/control_conv_rates_m_all_with_iteration.png"),        # path to your image
