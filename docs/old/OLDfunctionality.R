...
change_comparer <- function(X_new, X_old, time_step, time_seq, weights, C, relative = TRUE) {
  XX <- X_new - X_old
...
solve_coupled_system_multi_tol <- function(
  ...
  repeat {
    ...
    V_mat <- R %*% Psi %*% U_mat
    Q_mat <- solve_backward_evolution(my_op_frac, time_step, time_seq, RHST = V_mat - V_d)
    P_mat <- Q_mat
    z_new <- pmax(A, pmin(B, - P_mat / mu))
    Z_mat <- R %*% Psi %*% z_new
    
    # relative changes
    rel_changes_Z <- change_comparer(z_new, z_prev, time_step, time_seq, weights, C, relative = TRUE)  
    rel_changes_U <- change_comparer(U_mat, U_prev, time_step, time_seq, weights, C, relative = TRUE)
    rel_changes_P <- change_comparer(P_mat, P_prev, time_step, time_seq, weights, C, relative = TRUE)
    abs_changes_Z <- change_comparer(z_new, true_sol$z_bar, time_step, time_seq,  weights, C, relative = FALSE)
    abs_changes_U <- change_comparer(U_mat, true_sol$u_bar, time_step, time_seq,  weights, C, relative = FALSE)
    abs_changes_P <- change_comparer(P_mat, true_sol$p_bar, time_step, time_seq,  weights, C, relative = FALSE)
