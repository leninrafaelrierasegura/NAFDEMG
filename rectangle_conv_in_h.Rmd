---
title: "Convergence in 𝘩, rectangle case"
...
h_vector <- sqrt(a*b)*10^seq(log10(0.009), log10(0.1), length.out = 7)
...
save(errors_projected, file = here::here("data_files/errors_projected_h_square.RData"))
```


```{r}
# Load the errors_projected data
load(here::here("data_files/errors_projected_h_square.RData"))
```
...
p_h_square <- error.convergence.plotter(x_axis_vector = h_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
p_h_nicer_square <- errorConvergencePlotterLatex(x_axis_vector = h_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
ggsave(here::here("data_files/conv_rates_h_square.png"), width = 4, height = 5, plot = p_h_square, dpi = 300)
myggsave(p_h_nicer_square, width = 4, height = 5, use_mathpazo = TRUE)
save(p_h_square, file = here::here("data_files/p_h_square.RData"))
save(p_h_nicer_square, file = here::here("data_files/p_h_nicer_square.RData"))
```


```{r, eval = TRUE, fig.align='center', out.width="400px", out.height="500px", fig.cap = captioner("Comparison between theoretical and observed convergence behavior for the $L_2([0,1]^2\\times(0,T))$-error with respect to $h$ on a $\\text{log}_{10}$–$\\text{log}_{10}$ scale. Dashed lines indicate the theoretical rates, and solid lines represent the observed error curves. The legend below each plot shows the value of $\\alpha$ along with the corresponding theoretical ('theo'), and observed ('obs') rates for each case.")}
knitr::include_graphics(here::here("data_files/tikzpic/p_h_nicer_square_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = "data_files/conv_rates_h_square.png",        # path to your image
