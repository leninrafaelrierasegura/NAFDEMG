...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(tau_vector)) {
    ...
    h <- (time_step/abs(log(time_step)))^(1/alpha)
    m <- min(4, ceiling((log(h^2 * abs(log(time_step))))^2 / (4 * pi^2 * (1 - alpha / 2)))) #min(4, ceiling((log(h))^2 / (pi^2 * (1 - alpha / 2)))) 
...
save(errors_projected, file = here::here("old/errors_projected_tau_with_logtau.RData"))
```
...
ggsave(here::here("old/conv_rates_tau_tadpole.png"), width = 4, height = 5, plot = p_tau_tadpole, dpi = 300)
...
# option 1
slackr_upload(
  filename = here::here("old/conv_rates_tau_tadpole.png"),        # path to your image
