...
C_c <- max(outer(h_vector, alpha_vector,
               function(h, alpha) 1 + log(2) - alpha * log(h))) + 1e-1

C_vector <- sapply(alpha_vector, function(alpha) {
  max(1 + log(2) - alpha * log(h_vector)) + 1e-1
})
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(h_vector)) {
    h <- h_vector[i]
    time_step <- tau_from_h(h, alpha, C_c)#h^(2*alpha) #1e-5 #tau_from_h_exact(h, alpha) #h^alpha
    m <- min(8, m_from_h(h, alpha, C_c, time_step)) 
...
ggsave(here::here("old/conv_rates_h_tadpole.png"), width = 4, height = 5, plot = p_h_tadpole, dpi = 300)
...
# option 1
slackr_upload(
  filename = here::here("old/conv_rates_h_tadpole.png"),        # path to your image
