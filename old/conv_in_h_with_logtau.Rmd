...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(h_vector)) {
    h <- h_vector[i]
    time_step <- tau_from_h_exact(h, alpha) #h^alpha
    m <- min(4, ceiling((log(h))^2 / (pi^2 * (1 - alpha / 2)))) 
...
save(errors_projected, file = here::here("old/errors_projected_h_with_logtau.RData"))
```
...
p_h_nicer_tadpole <- errorConvergencePlotterLatex(x_axis_vector = h_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
ggsave(here::here("data_files/conv_rates_h_tadpole.png"), width = 4, height = 5, plot = p_h_tadpole, dpi = 300)
myggsave(p_h_nicer_tadpole, width = 4, height = 5, use_mathpazo = TRUE)
save(p_h_tadpole, file = here::here("data_files/p_h_tadpole.RData"))
save(p_h_nicer_tadpole, file = here::here("data_files/p_h_nicer_tadpole.RData"))
```
...
knitr::include_graphics(here::here("data_files/tikzpic/p_h_nicer_tadpole_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = "data_files/conv_rates_h_tadpole.png",        # path to your image
