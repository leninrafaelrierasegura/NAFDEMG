...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    U_approx <- solve_forward_evolution(my_op_frac, time_step, time_seq, RHST = FF_approx, val_at_0 = U_0)
...
save(errors_projected, file = here::here("data_files/new_errors_projected_m.RData"))
```


```{r}
load(here::here("data_files/new_errors_projected_m.RData"))
```
...
```{r}
ggsave(here::here("data_files/new_conv_rates_m.png"), width = 4, height = 5, plot = p_m, dpi = 300)
...
# option 1
slackr_upload(
  filename = "data_files/new_conv_rates_m.png",        # path to your image
