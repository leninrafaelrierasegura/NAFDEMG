...
#               )) 
# }
compute.partial.fraction.param <- function(p_coeff,
                                           q_coeff,
                                           time_step, # \tau
                                           scaling # \kappa^{2\beta}
                                           ) {
  res <- gsignal::residue(p_coeff, p_coeff + scaling * time_step * q_coeff)
  return(list(k = res$k, # remainder r
              r = res$r, # residues \{a_k\}_{k=1}^{m+1}
              p = res$p # poles \{p_k\}_{k=1}^{m+1}
              )) 
}
```
...
my.fractional.operators.frac <- function(L, # Laplacian matrix
  ...
  if(beta == 1){
    ...
    res <- rSPDE:::interp_rational_coefficients(order = m, type_rational_approx = "brasil", type_interp = "spline", alpha = beta)
    pol <- gsignal::rresidue(res$r, res$p, res$k, tol = 0.001)
    poles_rs_k <- compute.partial.fraction.param(p_coeff = pol$b, q_coeff = pol$a, time_step = time_step, scaling = scaling)
    ...
    partial_fraction_terms[[m+1]] <- C/poles_rs_k$k
    return(list(C = C, # mass matrix
                L = L, # Laplacian matrix scaled
                m = m, # rational order
                beta = beta, # smoothness parameter
...
my.solver.frac <- function(obj, # object returned by my.fractional.operators.frac()
  ...
  if (beta == 1){
    ...
    output <- solve(partial_fraction_terms[[m+1]], v)
    for (i in 1:m) {output <- output + solve(partial_fraction_terms[[i]], v)}
