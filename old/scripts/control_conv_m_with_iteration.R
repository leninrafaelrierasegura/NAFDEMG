...
# Parameters
T_final <- 2
kappa <- 16 # readRDS("old/kappa.RDS") #4
divider <- 1 # readRDS("old/divider.RDS") 
mu <- 0.1
a <- - 0.5*1
b <- 0.5*1
...
overkill_A <- matrix(a, nrow = nrow(overkill_C), ncol = length(overkill_time_seq))
overkill_B <- matrix(b, nrow = nrow(overkill_C), ncol = length(overkill_time_seq))
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    A_aux <- matrix(a, nrow = nrow(overkill_C), ncol = length(time_seq))
    B_aux <- matrix(b, nrow = nrow(overkill_C), ncol = length(time_seq))
    z_aux <- pmax(A_aux, pmin(B_aux, overkill_elliptic_v %*% t(phi)))
    ...
    errors_u_bar[i,j] <- sqrt(as.double(overkill_time_step * sum(UU * (overkill_C %*% UU))))
    errors_p_bar[i,j] <- sqrt(as.double(overkill_time_step * sum(PP * (overkill_C %*% PP))))
    errors_z_bar[i,j] <- sqrt(as.double(overkill_time_step * sum(ZZ * (overkill_C %*% ZZ))))
