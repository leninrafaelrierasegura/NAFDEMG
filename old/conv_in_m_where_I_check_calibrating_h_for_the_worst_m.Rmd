...
H_AUX <- c()
tau_aux <- c()
hs_calibrated <- rep(NA, length(alpha_vector))
for (j in 1:length(alpha_vector)) {
  alpha <- alpha_vector[j]
  for (i in 1:length(m_vector)) {
    m <- m_vector[i]
  ...
  hs_calibrated[j] <- h_aux
  }
min_h_coarse <- min(H_AUX)
min_h_coarse
min_tau <- min(tau_aux)
min_tau
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    #h <- exp(- pi * sqrt((1 - alpha / 2) * m))/divider 
    time_step <- (h^alpha)
    # h <- min_h_coarse #largest_nested_h(min_hs_trunc, h)
