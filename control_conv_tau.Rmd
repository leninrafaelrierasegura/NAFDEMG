...
errors <- matrix(NA, nrow = length(tau_vector), ncol = length(alpha_vector))
...
save(errors, file = here::here("data_files/control_error_tau.RData"))
```
...
for (u in 1:length(alpha_vector)) {
  observed_rates[u] <- coef(lm(log10(errors[, u]) ~ log10(tau_vector)))[2]
}
...
df <- as.data.frame(cbind(tau_vector, errors))
...
```{r}
ggsave(here::here("data_files/control_conv_rates_tau.png"), width = 4, height = 5, plot = p, dpi = 300)
```
