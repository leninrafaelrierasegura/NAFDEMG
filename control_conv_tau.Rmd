...
overkill_psi <- exp(-3*overkill_time_seq)
overkill_psi_prime <- -3 * exp(-3*overkill_time_seq)
overkill_phi <- 10 * exp(-3*overkill_time_seq) - 10*exp(-3*T_final)
overkill_phi_prime <- -30 * exp(-3*overkill_time_seq)
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(tau_vector)) {
    ...
    psi <- exp(-3*time_seq)
    psi_prime <- - 3 * exp(-3*time_seq)
    phi <- 10 * exp(-3*time_seq) - 10 * exp(-3*T_final)
    phi_prime <- - 30 * exp(-3*time_seq)
...
ggsave(here::here("data_files/control_conv_rates_tau_all.png"), width = 12, height = 6, plot = p_all, dpi = 300)
```
