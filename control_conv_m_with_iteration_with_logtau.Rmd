...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    UU <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (h^(alpha - 2))
    
    auxX <- res$P
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_p_bar - auxX
    PP <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (h^(alpha - 2))
    
    auxX <- res$Z
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_z_bar - auxX
    ZZ <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (h^(alpha - 2))
    ...
    slackr_msg(text = paste0("m =", m, ", alpha =", alpha, ", h =", h, ", time_step =", time_step, ", converged = ", res$converged, ", iterations = ", res$iterations, ", error u = ", UU, ", error p = ", PP, ", error z = ", ZZ, ", maxabs = ", maxabs), channel = "#research")
  }
}
save(errors_u_bar, errors_p_bar, errors_z_bar, file = here::here("data_files/control_error_m_with_iteration_with_logtau.RData"))
```
...
load(here::here("data_files/control_error_m_with_iteration_with_logtau.RData"))
```
...
p_u_bar_with_logtau <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
...
p_p_bar_with_logtau <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_p_bar, 
                               theoretical_rates, 
...
p_z_bar_with_logtau <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
...
p_m_u_with_logtau <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
                               observed_rates_u_bar,
...
p_m_u_nicer_tadpole_with_logtau <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_u_bar, 
                                 theoretical_rates, 
                                 observed_rates_u_bar,
...
p_m_z_with_logtau <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
                               observed_rates_z_bar,
...
p_m_z_nicer_tadpole_with_logtau <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_z_bar, 
                                 theoretical_rates, 
                                 observed_rates_z_bar,
...
p_all_m_with_logtau <- (p_u_bar_with_logtau | p_p_bar_with_logtau | p_z_bar_with_logtau) + 
  plot_annotation(
...
myggsave(p_m_u_nicer_tadpole_with_logtau, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_u_nicer_tadpole_with_logtau, file = here::here("data_files/p_m_u_nicer_tadpole_with_logtau.RData"))

myggsave(p_m_z_nicer_tadpole_with_logtau, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_z_nicer_tadpole_with_logtau, file = here::here("data_files/p_m_z_nicer_tadpole_with_logtau.RData"))


save(p_m_u_with_logtau, file = here::here("data_files/p_m_u_with_logtau.RData"))
save(p_m_z_with_logtau, file = here::here("data_files/p_m_z_with_logtau.RData"))

ggsave(here::here("data_files/control_conv_rates_m_all_with_iteration_with_logtau.png"), width = 12, height = 6, plot = p_all_m_with_logtau, dpi = 300)
```
...
knitr::include_graphics(here::here("data_files/control_conv_rates_m_all_with_iteration_with_logtau.png"))
```
...
# option 1
slackr_upload(
  filename = "data_files/control_conv_rates_m_all_with_iteration_with_logtau.png",        # path to your image
