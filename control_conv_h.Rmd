...
errors <- matrix(NA, nrow = length(h_vector), ncol = length(alpha_vector))
...
save(errors, file = here::here("data_files/control_error_h.RData"))
```
...
for (u in 1:length(alpha_vector)) {
  observed_rates[u] <- coef(lm(log10(errors[, u]) ~ log10(h_vector)))[2]
}
...
df <- as.data.frame(cbind(h_vector, errors))
...
```{r}
ggsave(here::here("data_files/control_conv_rates_h.png"), width = 4, height = 5, plot = p, dpi = 300)
```
