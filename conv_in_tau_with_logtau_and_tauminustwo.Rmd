...
::: {.custom-box}
To observe convergence in $\tau$, we choose $h \propto (\tau/|\ln(\tau)|)^{1/(\alpha-\epsilon)}$ and $m\propto\left\lceil\ln^2\!\bigl(h^{2-\epsilon}|\ln(\tau)|\tau^{2}\bigr)\big/\!\left(4\pi^2\left(1-\alpha/2\right)\right)\right\rceil$. This yields $E_\tau = E\lesssim\tau$, corresponding to a convergence rate of order $1$ with respect to the time step $\tau$. Fitting the regression $\log_{10}E_\tau=s_\tau\log_{10}\tau+\log_{10}c_\tau$, we therefore expect $s_\tau\approx1$.
:::
...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(tau_vector)) {
    ...
    m <- min(4, ceiling((log(h^(2 - epsilon) * time_step^2 * abs(log(time_step))))^2 / (4 * pi^2 * (1 - alpha / 2)))) #min(4, ceiling((log(h))^2 / (pi^2 * (1 - alpha / 2)))) 
...
save(errors_projected, file = here::here("data_files/errors_projected_tau_with_logtau_and_tauminustwo.RData"))
```


```{r}
# Load the errors_projected data
load(here::here("data_files/errors_projected_tau_with_logtau_and_tauminustwo.RData"))
```
...
p_tau_tadpole_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = tau_vector, 
                                   alpha_vector, 
                                   errors_projected, 
                                   theoretical_rates, 
                                   observed_rates,
...
p_tau_nicer_tadpole_with_logtau_and_tauminustwo <- errorConvergencePlotterLatex(x_axis_vector = tau_vector, 
                                 alpha_vector, 
                                 errors_projected, 
                                 theoretical_rates, 
                                 observed_rates,
...
ggsave(here::here("data_files/conv_rates_tau_tadpole_with_logtau_and_tauminustwo.png"), width = 4, height = 5, plot = p_tau_tadpole_with_logtau_and_tauminustwo, dpi = 300)
myggsave(p_tau_nicer_tadpole_with_logtau_and_tauminustwo, width = 4, height = 5, use_mathpazo = TRUE)
save(p_tau_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_tadpole_with_logtau_and_tauminustwo.RData"))
save(p_tau_nicer_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_tau_nicer_tadpole_with_logtau_and_tauminustwo.RData"))
...
knitr::include_graphics(here::here("data_files/tikzpic/p_tau_nicer_tadpole_with_logtau_and_tauminustwo_with_mathpazo.pdf"))
```
...
# option 1
slackr_upload(
  filename = here::here("data_files/conv_rates_tau_tadpole_with_logtau_and_tauminustwo.png"),        # path to your image
