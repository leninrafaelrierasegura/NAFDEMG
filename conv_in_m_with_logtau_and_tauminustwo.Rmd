...
::: {.custom-box}
To observe convergence in $m$, we determine $h$ numerically from the relation $|\ln((C_mh^{\alpha-2})^{1/3})| \propto C_m^{1/3}h^{\epsilon-(2\alpha+2)/3}$ and choose $\tau \propto (C_mh^{\alpha-2})^{1/3}$, where $C_m = \mathrm{e}^{-2\pi\sqrt{(1-\alpha/2)m}}$. This yields $E_m = E/(\tau^{-2}h^{\alpha-2})\lesssim C_m$, corresponding to exponential decay with respect to the rational approximation order $m$. Fitting the regression $\ln E_m = s_m\sqrt{m} +\ln c_m$, we therefore expect $s_m\approx -2\pi\sqrt{1-\alpha/2}$.
:::
