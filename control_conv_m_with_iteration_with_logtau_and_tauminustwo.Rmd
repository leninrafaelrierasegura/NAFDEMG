...
for (j in 1:length(alpha_vector)) {
  ...
  for (i in 1:length(m_vector)) {
    ...
    UU <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (time_step^(-2) * h^(alpha - 2))
    
    auxX <- res$P
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_p_bar - auxX
    PP <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (time_step^(-2) * h^(alpha - 2))
    
    auxX <- res$Z
    auxX <- construct_piecewise_projection(Psi %*% auxX, time_seq, overkill_time_seq)
    auxX <- overkill_z_bar - auxX
    ZZ <- sqrt(as.double(overkill_time_step * sum(auxX * (overkill_C %*% auxX)))) / (time_step^(-2) * h^(alpha - 2))
...
save(errors_u_bar, errors_p_bar, errors_z_bar, file = here::here("data_files/control_error_m_with_iteration_with_logtau_and_tauminustwo.RData"))
```
...
load(here::here("data_files/control_error_m_with_iteration_with_logtau_and_tauminustwo.RData"))
```
...
p_u_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
...
p_p_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_p_bar, 
                               theoretical_rates, 
...
p_z_bar_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
...
p_m_u_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_u_bar, 
                               theoretical_rates, 
                               observed_rates_u_bar,
...
p_m_u_nicer_tadpole_with_logtau_and_tauminustwo <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_u_bar, 
                                 theoretical_rates, 
                                 observed_rates_u_bar,
                                 ...
                                 y_axis_label = "$\\|\\bar{u}- \\bar{U}_{h,m}^{\\tau}\\|_{L_2(\\Gamma\\times[0,T])}\\quad(\\log_{\\mathrm{e}}\\mbox{ scale})$",
                                 color_label = "$\\quad\\quad\\;\\;\\alpha\\;|\\;\\phantom{-}\\mathtt{theo}\\;|\\;\\;\\phantom{-}\\mathtt{obs}\\;\\;|\\%\\mathtt{re}$",
                                 apply_sqrt = TRUE)

p_m_z_with_logtau_and_tauminustwo <- error.convergence.plotter(x_axis_vector = m_vector, 
                               alpha_vector, 
                               errors_z_bar, 
                               theoretical_rates, 
                               observed_rates_z_bar,
...
p_m_z_nicer_tadpole_with_logtau_and_tauminustwo <- errorConvergencePlotterLatex(x_axis_vector = m_vector, 
                                 alpha_vector, 
                                 errors_z_bar, 
                                 theoretical_rates, 
                                 observed_rates_z_bar,
                                 ...
                                 fig_title = "$\\mbox{Convergence in }m$",
                                 x_axis_label = "$m\\quad(\\sqrt{\\phantom{x}}\\mbox{ scale})$",
                                 y_axis_label = "$\\|\\bar{z}- \\bar{\\mathcal{Z}}_{h,m}^{\\tau}\\|_{L_2(\\Gamma\\times[0,T])}\\quad(\\log_{\\mathrm{e}}\\mbox{ scale})$",
                                 color_label = "$\\quad\\quad\\;\\;\\alpha\\;|\\;\\phantom{-}\\mathtt{theo}\\;|\\;\\;\\phantom{-}\\mathtt{obs}\\;\\;|\\%\\mathtt{re}$",
                                 apply_sqrt = TRUE)

p_all_m_with_logtau_and_tauminustwo <- (p_u_bar_with_logtau_and_tauminustwo | p_p_bar_with_logtau_and_tauminustwo | p_z_bar_with_logtau_and_tauminustwo) + 
  plot_annotation(
...
myggsave(p_m_u_nicer_tadpole_with_logtau_and_tauminustwo, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_u_nicer_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_m_u_nicer_tadpole_with_logtau_and_tauminustwo.RData"))

myggsave(p_m_z_nicer_tadpole_with_logtau_and_tauminustwo, width = 4, height = 5, use_mathpazo = TRUE)
save(p_m_z_nicer_tadpole_with_logtau_and_tauminustwo, file = here::here("data_files/p_m_z_nicer_tadpole_with_logtau_and_tauminustwo.RData"))


save(p_m_u_with_logtau_and_tauminustwo, file = here::here("data_files/p_m_u_with_logtau_and_tauminustwo.RData"))
save(p_m_z_with_logtau_and_tauminustwo, file = here::here("data_files/p_m_z_with_logtau_and_tauminustwo.RData"))

ggsave(here::here("data_files/control_conv_rates_m_all_with_iteration_with_logtau_and_tauminustwo.png"), width = 12, height = 6, plot = p_all_m_with_logtau_and_tauminustwo, dpi = 300)
```

```{r, eval = TRUE, fig.align='center', fig.dim= c(12,6), fig.cap = captioner("Comparison of theoretical and observed convergence behavior for the $L_2((0,T);L_2(\\Gamma))$-error with respect to $m$ on a semi-$\\text{log}_{e}$ scale, with $m$ plotted on a square-root scale. Dashed lines indicate the theoretical rates, and solid lines represent the observed error curves. The legend below each plot shows the value of $\\alpha$ along with the corresponding theoretical ('theo'), and observed ('obs') rates for each case.")}
...
knitr::include_graphics(here::here("data_files/control_conv_rates_m_all_with_iteration_with_logtau_and_tauminustwo.png"))
```
...
# option 1
slackr_upload(
  filename = "data_files/control_conv_rates_m_all_with_iteration_with_logtau_and_tauminustwo.png",        # path to your image
